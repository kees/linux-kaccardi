...
static void randomize_text(struct module *mod, struct load_info *info)
{
	...
	text_list = kmalloc(sizeof(*text_list) * max_sections, GFP_KERNEL);
	if (text_list == NULL)
		return;
	...
	// get new set of offsets.
	// XXX remove debug stuff here eventually.
	for (i = 0; i < num_text_sections; i++) {
		...
		printk("old offset for %s: 0x%llx\n", sname, shdr->sh_entsize);
		shdr->sh_entsize = get_offset(mod, &size, shdr, infosec);
		printk("new offset for %s: 0x%llx\n", sname, shdr->sh_entsize);
	}
